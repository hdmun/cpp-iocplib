...
namespace iocplib {
	...
	{
		int bind_ = ::bind( handle, pAddr, len );
	...
	{
		int bind_ = ::listen( handle, backlog );
		if ( bind_ == SOCKET_ERROR ) {
	...
	{
		int ret = ::connect( handle, pAddr, sizeof( *pAddr ) );
	...
	{
		int ret = ::setsockopt( handle, SOL_SOCKET, SO_UPDATE_ACCEPT_CONTEXT,
	...
	void WinSock::SetNonBlocking( bool nonblocking )
	{
		...
		int ret = ::ioctlsocket( handle, FIONBIO, &b );
		if ( ret == SOCKET_ERROR ) {
	...
	{
		int n = size;
		int ret = ::setsockopt( handle, SOL_SOCKET, SO_RCVBUF,
	...
	{
		int n = size;
		int ret = ::setsockopt( handle, SOL_SOCKET, SO_SNDBUF,
			reinterpret_cast<const char*>(&n), sizeof( n ) );
		if ( ret == SOCKET_ERROR ) {
